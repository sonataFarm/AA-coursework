...
def colleagues_of_garfunkel
  ...
  # select all actors in movies with id in (all movies of simon garfunkel)
  # except actors with name 'simon garfunkel'
  #
  execute(<<-SQL)
    SELECT actors.name
    FROM actors
    JOIN castings ON castings.actor_id = actors.id
    WHERE castings.movie_id IN (SELECT DISTINCT movies.id
                                FROM movies
                                JOIN castings on castings.movie_id = movies.id
